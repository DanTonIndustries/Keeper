 ...
 */
public class UpdateLocalUsersAsync extends AsyncTask<Void, Void, String> {
...
/*    @Override
    ...
    protected void onPostExecute(String json) {
        ...
        Gson gson = new Gson();
        UserEntity[] userArray = gson.fromJson(json, UserEntity[].class);

        //TODO: For each table drop and replace contents with the remote.
        SQLiteDatabase db = DatabaseManager.getInstance().openDatabase();
        db.execSQL("DELETE FROM " + KeeperContract.UserEntry.TABLE_NAME);
        for (UserEntity user : userArray) {
            user.addToLocalDb(db);
        }
